...
function SPKSegmentType1(array, head::SPKSegmentHeader, lend::Bool)

    # number of records in segment
    n = Int(get_float(array, 8*(head.faa-1), lend))

    ndirs = n ÷ 100

    SPKSegmentType1(
        n, ndirs, Int[0.0], zeros(15), zeros(3), zeros(3), zeros(15, 3), 
...
function get_coefficients!(array, head::SPKSegmentHeader, seg::SPKSegmentType1, 
            index::Integer, lend::Bool)

    i0 = 8*(head.iaa - 1) + 8*71*(index-1)
    ...
    for k = 1:3 
        for p = 1:15 
            seg.dt[p, k] = get_float(array, i3 + 8*(p-1) + 8*15*(k-1), lend)
        end
    end
...
function compute_mda3(seg::SPKSegmentType1, time::Number)
...
function vector3(array, daf::DAFHeader, head::SPKSegmentHeader, 
    seg::SPKSegmentType1, time::Number)
