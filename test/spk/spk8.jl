...
@testset "SPK Type 8" verbose=true begin 
    ...
    for kernel in kernels
        ...
        for j in 1:2000
            ...
            @test yj1 ≈ yc1 atol=1e-9 rtol=1e-10
            @test yj2 ≈ yc2 atol=1e-9 rtol=1e-10
            @test yj3 ≈ yc3 atol=1e-9 rtol=1e-10
            @test yj4 ≈ yc4 atol=1e-9 rtol=1e-10
            ...
            # Position (can't compare the position because it has different coefficients)
            # @test D¹(t->ephem_vector3(ephj, cid, tid, t), tj) ≈ yj4[4:6] atol=1e-9 rtol=1e-5
            # @test D²(t->ephem_vector3(ephj, cid, tid, t), tj) ≈ yj4[7:9] atol=1e-9 rtol=1e-5
            # @test D³(t->ephem_vector3(ephj, cid, tid, t), tj) ≈ yj4[10:12] atol=1e-9 rtol=1e-5

            # Velocity (these ensure the acceleration\jerk are computed correctly)
            ...
            # Acceleration (this ensures the acceleration\jerk are computed correctly)
