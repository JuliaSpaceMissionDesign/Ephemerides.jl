...
@testset "SPK Type 9" verbose=true begin 
    ...
    kernel = joinpath(test_dir, "example1spk_seg9.bsp")
    ...
    for _ in 1:1000
        ...
        # Velocity 
        @test D¹(t->ephem_vector6(ephj, cid, tid, t), tj)[4:6] ≈ yj4[7:9] atol=1e-9 rtol=1e-9
        @test D²(t->ephem_vector6(ephj, cid, tid, t), tj)[4:6] ≈ yj4[10:12] atol=1e-9 rtol=1e-9

        # Acceleration 
        @test D¹(t->ephem_vector9(ephj, cid, tid, t), tj)[4:9] ≈ yj4[7:12] atol=1e-9 rtol=1e-9
